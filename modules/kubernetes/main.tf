...
data "template_file" "public_subnet_map" {
  count    = "4"
  template = "${file("${path.module}/templates/kops/subnet.yaml")}"
...
data "template_file" "node_group_definitions" {
  ...
  template = "${file("${path.module}/templates/kops/agents.yaml")}"
...
data "template_file" "addons" {
  ...
  template = "${file("${path.module}/templates/kops/addons.yaml")}"
...
data "template_file" "kops_values_file" {
  template = "${file("${path.module}/templates/kops/values.yaml")}"
...
resource "null_resource" "provision_kops" {
  ...
  provisioner "local-exec" {
    ...
    command = <<EOT
    kops toolbox template --template ${path.module}/templates/kops/cluster.yaml \
    --template ${path.module}/templates/kops/worker.yaml \
    --template ${path.module}/templates/kops/master.yaml --values ${path.root}/output/values-rendered.yaml > ${path.root}/output/output.yaml
...
EOT
  ...
  provisioner "local-exec" {
    ...
    command = "kops delete cluster ${local.cluster_name}.${var.kops_cluster.dns_zone} --yes"
  }
}
